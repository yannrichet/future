...
options(mc.cores = NULL, future.plan=NULL, future.cmdargs=c(sprintf("--parallel=%d", maxCores)))
args <- parseCmdArgs()
str(args)
stopifnot(args$p == maxCores)

options(mc.cores = NULL, future.plan=NULL, future.cmdargs=c("-p", 1L, sprintf("--parallel=%d", maxCores)))
args <- parseCmdArgs()
str(args)
stopifnot(args$p == maxCores)

options(mc.cores = NULL, future.plan=NULL, future.cmdargs=c("-p", 0L))
args <- parseCmdArgs()
stopifnot(is.null(args$p))
res <- tryCatch(parseCmdArgs(), warning=function(w) w)
stopifnot(inherits(res, "warning"))

options(mc.cores = NULL, future.plan=NULL, future.cmdargs=c("-p", .Machine$integer.max))
args <- parseCmdArgs()
stopifnot(is.null(args$p))
res <- tryCatch(parseCmdArgs(), warning=function(w) w)
stopifnot(inherits(res, "warning"))

options(mc.cores = NULL, future.plan=NULL, future.cmdargs=NULL)
...
options(mc.cores = NULL, future.plan=NULL, future.cmdargs=c("-p", 1))
.onLoad(pkgname, pkgname)
strategy <- plan()
print(strategy)
message("nbrOfWorkers(): ", nbrOfWorkers())
...
plan("default")
options(mc.cores = NULL, future.plan=NULL, future.cmdargs="-parallel=1")
.onLoad(pkgname, pkgname)
strategy <- plan()
print(strategy)
message("nbrOfWorkers(): ", nbrOfWorkers())
...
options(mc.cores = NULL, future.plan=NULL, future.cmdargs=c("-p", 2))
.onLoad(pkgname, pkgname)
strategy <- plan()
print(strategy)
message("nbrOfWorkers(): ", nbrOfWorkers())
...
options(mc.cores = NULL, future.plan=NULL, future.cmdargs=c("-p", 0))
.onLoad(pkgname, pkgname)
strategy <- plan()
print(strategy)
message("nbrOfWorkers(): ", nbrOfWorkers())
...
message("- .onLoad() w/ -p -1 ...")
options(mc.cores = NULL, future.plan=NULL, future.cmdargs=c("-p", -1))
nbrOfCores <- availableCores()
.onLoad(pkgname, pkgname)
strategy <- plan()
print(strategy)
print(nbrOfWorkers())
stopifnot(nbrOfWorkers() == nbrOfCores - 1L)
plan("default")
message("- .onLoad() w/ -p -1 ... DONE")
...
print(strategy)
print(nbrOfWorkers())
stopifnot(nbrOfWorkers() == nbrOfCores)
plan("default")
message("- .onLoad() w/ -p -0 ... DONE")
...
options(mc.cores = NULL, future.plan=NULL, future.plan='multisession', future.cmdargs=NULL)
.onLoad(pkgname, pkgname)
strategy <- plan()
print(strategy)
print(nbrOfWorkers())
...
options(mc.cores = NULL, future.plan='multisession', future.cmdargs=c("-p", "1"))
.onLoad(pkgname, pkgname)
strategy <- plan()
print(strategy)
print(nbrOfWorkers())
...
options(mc.cores = NULL, future.availableCores.system=1L)
.onLoad(pkgname, pkgname)
options(mc.cores = NULL, future.availableCores.system=NULL)
...
options(mc.cores = NULL, future.plan=NULL, future.cmdargs=NULL, future.availableCores.system=1L)

options(mc.cores = NULL, future.plan=NULL, future.cmdargs=NULL, future.availableCores.system=NULL)
...
for (value in list(NULL, FALSE, TRUE)) {
  options(mc.cores = NULL, future.startup.loadScript=value)

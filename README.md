...
[1] 31620
```
As seen by the different process IDs, future `a` is evaluated sequentially in the same process as the calling environment whereas the other two are evaluated using multiprocess futures.
...
> b
 ...
 31535  31625  31626 
```
which clearly show that `a` and `b` are resolved in the calling process (pid 31535) whereas the two nested futures (`b1` and `b2`) are resolved in two separate R processes (pids 31625 and 31626).
...
> b
 ...
 31628  31629  31631 
```
First, we see that both `a` and `b` are resolved in different processes (pids 31627 and 31628) than the calling process (pid 31535).  Second, the two nested futures (`b1` and `b2`) are resolved in yet two other R processes (pids 31629 and 31631).
...
Then try with multiprocess evaluation, which calculates the different Mandelbrot planes using parallel R processes running in the background.  Use,
```r
plan(multiprocess)
demo("mandelbrot", package="future", ask=FALSE)
```
